...
def predict():
    ...
    explanation = build_explanation(result["prediction"], message)
...
def history():
    ...
    if q:
        history_rows = [row for row in history_rows if q in row["message_text"].lower()]

    if label_filter in ["Spam", "Not Spam"]:
        history_rows = [row for row in history_rows if row["predicted_label"] == label_filter]
